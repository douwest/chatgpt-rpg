...
- All entities which can be interacted with, such as npc's, enemies, loot, world objects,  locations, should be inserted into the completion as json strings (must be VALID json, being able to be parsed by JSON.parse() in javascript) containing the following information: { type: Type, name: string, description: string, rarity?: Rarity }, where Type is one of the following: ['enemy', 'npc', 'item', 'world_object', 'location']. Rarity is only used if the type is 'item', it can not be applied to any other type of entity. Rarity can be ['common', 'unusual', 'rare', 'epic', 'legendary']. Common items are the most common, ranging to legendary items which are incredibly rare.
